...
class Cmn_functs {
  ...
  // *************************************************************************
  // ******* 21 agosto 2020
  // ******* recalcula el saldo de cuenta del cliente a partir del 1 de enero 2020
  // *************************************************************************  
  function recalc_saldo($elm_id,$nro_rec){
    //*** 
    $prev_mov = $this->CMF->app_model->get_obj("SELECT saldo FROM comprobantes WHERE elements_id  = {$elm_id} AND nro_comprobante < {$nro_rec} ORDER BY id DESC limit 1 ");
    $next_movs_id = $this->CMF->app_model->get_arr("SELECT id,op_caja_nro,monto FROM comprobantes WHERE elements_id  = {$elm_id} AND nro_comprobante > {$nro_rec}  AND estado > -1 ORDER BY id");
    $crs = intval($prev_mov->saldo);  
    foreach ($next_movs_id as $key => $c) {
      if(intval($c['op_caja_nro']) > -1){
        //***** ES UN INGRESO
        $crs += intval($c['monto']);
      }else{
        $crs -= intval($c['monto']);
      ...
      $upd = ['saldo'=> $crs];
      $u = $this->CMF->app_model->update('comprobantes', $upd,'id', $c['id']);
    }
  }
  
  // *************************************************************************
  // ******* 24 agosto 2020
  // ******* cosntruye un query pivoteando una tabla desde struct para usarlo
  // ******* en pcles 
  // *************************************************************************  
  function create_query($tbl_type,$type_id){
    if($tbl_type == 'atom'){
      $qry = "SELECT atom_id as atom_id ";  
      $str = $this->CMF->app_model->get_arr("SELECT * FROM atoms_struct WHERE atom_types_id = {$type_id} ");
      foreach ($str as $sti) {
        $qry .= ", MAX(CASE WHEN label = '{$sti['label']}'  THEN value END) AS '{$sti['label']}' ";
      }
      $qry .= " FROM atoms_pcles WHERE atom_types_id = {$type_id} GROUP BY atom_id ";
      return $qry;
      
    }
    if($tbl_type == 'element'){
      $qry = "SELECT elements_id as elements_id ";  
      $str = $this->CMF->app_model->get_arr("SELECT * FROM elements_struct WHERE elements_types_id = {$type_id} ");
      foreach ($str as $sti) {
        $qry .= ", MAX(CASE WHEN label = '{$sti['label']}'  THEN value END) AS '{$sti['label']}' ";
      }
      $qry .= " FROM elements_pcles WHERE elements_types_id = {$type_id} GROUP BY elements_id ";
      return $qry;
    } 

...
class Tbox_1 extends CI_Controller {
 ...
 // <-- tester -->
	...
	CREATE OR REPLACE VIEW CONTRATO_2 AS SELECT
ep0.elements_id,
a2.name as barrio_id,
ep6.value as cant_ctas,
ep7.value as indac,
ep9.value as current_ciclo,
ep11.value as cant_ctas_restantes
FROM elements_pcles as ep0
INNER JOIN `elements_pcles` ep2 on ep2.elements_id = ep0.elements_id and ep2.struct_id = 74
INNER JOIN `atoms` a2 on a2.id = ep2.value
INNER JOIN `elements_pcles` ep6 on ep6.elements_id = ep0.elements_id and ep6.struct_id = 35
INNER JOIN `elements_pcles` ep7 on ep7.elements_id = ep0.elements_id and ep7.struct_id = 36
INNER JOIN `elements_pcles` ep9 on ep9.elements_id = ep0.elements_id and ep9.struct_id = 45
INNER JOIN `elements_pcles` ep11 on ep11.elements_id = ep0.elements_id and ep11.struct_id = 61
WHERE ep0.elements_types_id = 1 GROUP BY ep0.elements_id
	...
	*/
	...
	public function tft()
	{
		...
		var_dump($df->get_saldo_a_financiar());
	}
...
// 31/03/2020
	function venc(){
		$elm = new Element(6074);
		$v = $this->update_vencimientos($elm);
		var_dump($v);
	...
	function fix_etp(){
		$e = $this->Mdb->db->query("SELECT id from elements WHERE elements_types_id = 5 ");
	...
	function xtst(){
		// encuentra elements con cuotas a pagar no en mora
		$elms = $this->Mdb->db->query("SELECT id FROM `elements` WHERE id = 5568 ");
		$x = 0;
		// elements contrato
		if($elms->result_id->num_rows){
			foreach ($elms->result() as $e) {
				$data_refi_lote =[];
				$data_refi_srvc = [];
				$vrfc_lote = $this->verif_cnt($e->id);
				if($vrfc_lote){
					// create service refinanc lote
					echo "<br/>creating service refinac lote :".$vrfc_lote['owner']." fecha: ".$vrfc_lote['fecha']." Monto: ".$vrfc_lote['monto'];
					if(intval($vrfc_lote['monto'])>0){
						$data_refi_lote = [
							'elm_id'=>$e->id,
							'fields'=>[
								'atom_id'=>12658,
								...
								'monto_total'=>intval($vrfc_lote['monto']),
								'monto_cta_1'=>intval($vrfc_lote['monto'])/3
							]
						];
					}
					// busco si hay servicios para el curr contract
					$s = $this->Mdb->db->query("SELECT id FROM elements WHERE elements_types_id = 4 AND owner_id = {$e->id} ");
					if($s->result_id->num_rows){
						$tot = 0;
						foreach ($s->result_array() as $srv) {
							$vrfcs = $this->verif_cnt($srv['id']);
							if(intval($vrfcs['monto'])>0){
								$tot += intval($vrfcs['monto']);
							}
						}
						echo "<br/>creating service refinac servicios :".$vrfcs['owner']." fecha: ".$vrfcs['fecha']." Monto: ".$tot;
						if(intval($tot)>0){
							$data_refi_srvc = [
								'elm_id'=>$e->id,
								'fields'=>[
									...
									'cant_ctas'=>3,
									'cant_ctas_restantes'=>3,
									'monto_total'=>intval($tot),
									'monto_cta_1'=>intval(intval($tot)/3)
								]
							];
						}
					}
				...
				if(count($data_refi_lote) > 0){
					...
					$refi = new Atom(-1,"REFINANCIACION_ESPECIAL",$vrfc_lote['owner']);
					$refi->pcle_updv($refi->get_pcle('elem_id')->id,$e->id);
					$refi->pcle_updv($refi->get_pcle('refi_lote_id')->id,$refi_lote_id);
					if(count($data_refi_srvc)> 0){
						$refi_servicios_id = $this->save_new_service($data_refi_srvc);
						$refi->pcle_updv($refi->get_pcle('refi_servicios_id')->id,$refi_servicios_id);
					}
				}
			}
		}
	...
	// verifica si el element puede acceder a la refi
	// retorna el monto total a refinanciar
	function verif_cnt($e_id){
		$c = new Element($e_id);
		$ctavenc = $c->get_events(4,'a_pagar');
		// no tiene cuotas vencidas
		if(count($ctavenc['events']) == 0){
			if($c->get('type_id') == 1){
				$owner = (new Atom($c->get_pcle('prod_id')->value))->name;
				// echo "<br>".$owner;
			}
			else if($c->get('type_id') == 4){
				$owner = (new Atom($c->get_pcle('atom_id')->value))->name;
				// echo "<br>".$owner;
			}
			$ct_upc = $c->get_events(8,'a_pagar');
			if(!empty($ct_upc)){
				$ct_x = $c->get_cta_by_date('2020-04-10');
				//  tengo cuota de abril
				if(!empty($ct_x)){
					// echo " verify contrato: returning fecha ".$ct_x->get_pcle('fecha_vto')->value." monto: ".$ct_x->get_pcle('monto_cta')->value;
					// echo " <br/>proxima cuota fecha event ".$ct_x->get('date');
					return ['owner'=>$owner,'fecha'=>$ct_x->get('date'),'monto'=>$ct_x->get_pcle('monto_cta')->value];
				}
			}else{
				return false;
				// echo " CUOTA: ".$ct_upc['events'][0]['fecha'];
			}
		}
	}
	...
	function tmk_cat_view(){
		$x = 6; // obra element
		$s = $this->Mdb->db->query("SELECT label FROM elements_struct WHERE elements_types_id = $x ");
			//  query para hacer view
		$f = '';
		$l = '';
		// $q = "SELECT ";
		// foreach ($s->result() as $i =>$v) {
		// 	$f .= 'ep_'$i.','
		// }
	}
	...
	function fix_cli(){
		...
		$c = $this->Mdb->db->query("SELECT id from elements where elements_types_id = 1 limit 100");
  ...
  // *************************************************************************
    ...
    // *************************************************************************
     function call_new_atom(){
        ...
        $p = $this->input->post('data');
        $struct = $this->app_model->get_arr("SELECT label,value,title,dom_types,vis_ord_num FROM `elements_struct` WHERE elements_types_id = {$p['elements_types_id']} ORDER BY vis_ord_num ASC");
        if($struct){
    	...
    	function update_type(){
    		...
    		foreach ($lotes as $l) {
    			...
    			if(!empty($a->id) && !preg_match('/R_/' , $a->name)){
    				$pcles = $a->get_pcle('');
    				foreach ($pcles as $p) {
    					$a->set_pcle($p->id,$p->label,$p->value,$p->title,$p->dom_types,2);
    				}
    			}
    		}
    	}

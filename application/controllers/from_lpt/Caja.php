...
class Caja extends CI_Controller {
  ...
  public function index() {
    ...
    if (is_array($user)) {
      ...
      // var_dump($this->cmn_functs->get_cuentas_dpdown_data($this->usr_obj->permisos_usuario));
        $selects = [
        'cuentas'=>[['id'=>3,'lbl'=>'Caja Unica']],//$this->cmn_functs->get_cuentas_dpdown_data($this->usr_obj->permisos_usuario),
      ...
      $var=array(
          ...
          'selects'=>($this->usr_obj->permisos_usuario < 3)?$selects:[],
  ...
  function planilla_caja(){
    ...
    // foreach ($c as $key => $c_id) {
      ...
      $l = $this->get_listado_caja_2(3,$p['data']['fec_desde'],$p['data']['fec_hasta']);
      $cajas[] = ['caja_id'=>3,'list'=>$l];
    // ?}
  ...
  function edit_op(){
    ...
    $res = $this->app_model->get_obj("
        ...
        LEFT OUTER join atoms_pcles pl on a.venta_id = pl.atom_id AND pl.label = 'name'
  ...
  function movs_de_cajas(){
    ...
    $r = $this->app_model->get_arr("SELECT
      ...
      LEFT OUTER JOIN atoms atbr on atbr.id = ccd.barrio_id
      LEFT OUTER JOIN atoms atlt on atlt.id = ca.venta_id

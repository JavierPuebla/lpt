...
class Configuracion extends CI_Controller {
    ...
    if ( ! is_array($user)){
      ...
      $var=array(
        ...
        'selects'=>[
          ...
          'emprendimiento'=>$this->cmn_functs->get_dpdown_data('BARRIO'),
  ...
  function get_screen($u){
    ...
    $id = $u['user_id'];
    if($id == 484){return $b;}
    if($id == 503){return [$b[0],$b[1],$b[2],$b[3],$b[4]];}
    if($id != 503 && $id != 484){return [$b[0],$b[1],$b[2],$b[4]];}
    else{return false;}
  }
...
// }
  ...
  function list_contab(){
    ...
    $rows = [];
		foreach($tb_list as $tb_row){
          ...
          $pcle[]=['label'=>$key.'_'.$p['id'],'value'=>$item,'title'=>ucwords($key), 'dom_types'=>'3','elem_type_src'=> $p['id']];
        }else{
          $pcle[]=['label'=>$key,'value'=>$item,'title'=>ucwords($key), 'dom_types'=>'1'];
        }
  ...
  function save_atom(){
    ...
    foreach ($p['data'] as $key => $val) {
      $x = $t->get_pcle($key);
      if(!empty($x)){
        $t->set_pcle($x->id,$key,$val,$x->title,$x->dom_types);
      }else{
  ...
  function config_update_plan(){
    ...
    $rev_fp = Clientes::call_update_plan($e);
    if(!empty($rev_fp)){
      $r =[
      ...
      $r =[
        'tit'=>'Error',
        'msg'=>'Fecha de Revision incorrecta',
        'type'=>'danger',
        'container'=>'modal',
        'after_action'=>'back'
      ];
  ...
  function  old_edit_element($p=0){
    ...
    if($e->type === 'EMPTY'){
      ...
      $lote_nom = new Atom($lote_id);
      $cli_nom = new Atom($cli_id);
      $financ_nom = new Atom($financ_id);

      $lote = ['id'=>$lote_id,'title'=>'Codigo Lote ','value'=> $lote_nom->get_pcle('name')->value];
      $cli = ['id'=>$cli_id,'title'=>'Nombre del Titular','value'=>$cli_nom->name];
      $financ = ['id'=>$financ_id,'title'=>'Plan Financiacion','value'=> $financ_nom->get_pcle('name')->value];


      $tit_id = $e->get_pcle('cli_id');
      $fec_ini = $e->get_pcle('fec_ini');
      // $saldo = $e->get_pcle('saldo');
      $estado = $e->get_pcle('curr_state');
      $cant_ctas_post_posesion = $e->get_pcle('cant_ctas_post_posesion');
      $cuotas = $e->get_ctas_pagas();
      ...
      if(!empty($servicios)){
        foreach ($servicios as $srv) {
          ...
          $s[] = ['fec_ini' => $x->get_pcle('fec_ini')->value , 'name'=>$xs->get_pcle('name')->value , 'cuotas'=>$x->get_ctas_pagas()];
        }
      }
      $d = [
        'lote'=>$lote,
        ...
        'estado'=>$estado,
        'cuotas'=>$cuotas,
        'cant_servicios'=>count($s),
        'servicios'=>$s,
        'cant_ctas_post_posesion'=> $cant_ctas_post_posesion
      ];
      $r =[
        'sending'=>false,
        'method'=>'edit_element',
  ...
  // *****************************************************
  // **** nuevo editor de cotratos
  // *****************************************************
  function  edit_elm($p=0){
      // SI PARAM ESTA VACIO BUSCA EN $_POST
      if($p === 0){
        $p = $this->input->post('data');
      }
  ...
  function pcle_updv_cnfg($type=0,$prnt_id=0,$id=0,$v=0){
    if($type == 0 && $prnt_id == 0 && $id == 0 && $v == 0){
      $p = $this->input->post('data');
      $label = (array_key_exists('lid',$p))?substr($p['lid'],0,strrpos($p['id'],'_')):'';
    ...
    if($type && $prnt_id && $id && $v){
      $e = new $type($prnt_id);
      $e->pcle_updv($id,$v);
      // fix para cli_id y titular id
      if($label === 'titular_id'){
        $e->pcle_updv($e->get_pcle('cli_id')->value,$v);
      }
      // return 'ok';

    ...
    public function index() {
      ...
      if (is_array($user)) {
        ...
        $selects = [
          ...
          'owner_id'=>$this->cmn_functs->get_dpdown_data('CLIENTE'),
          ...
          // 'vendedor'=>$this->cmn_functs->get_dpdown_data('VENDEDOR'),
          'prod_id'=>$this->cmn_functs->get_dpdown_data('PRODUCTO'),
          ...
          'cant_ctas' => [['id'=>1,'lbl'=>'1'],['id'=>2,'lbl'=>'2'],['id'=>3,'lbl'=>'3'],['id'=>6,'lbl'=>'6'],['id'=>12,'lbl'=>'12'],['id'=>24,'lbl'=>'24'],['id'=>36,'lbl'=>'36'],['id'=>48,'lbl'=>'48'],['id'=>60,'lbl'=>'60'],['id'=>90,'lbl'=>'90'],['id'=>144,'lbl'=>'144'],['id'=>150,'lbl'=>'150'],['id'=>156,'lbl'=>'156'],['id'=>198,'lbl'=>'198'],['id'=>204,'lbl'=>'204']],
  ...
  // *** CREA EL SCREEN EN BASE A LOS PERMISOS DEL USUARIO
    function get_screen($u){
      $btns = [
        ...
        ['call'=>['method'=>'list','sending'=>true,'action'=>'call','type_text'=>'CLIENTE','data'=>0],'tag'=>'Listado de Ventas'],
        ['call'=>['method'=>'new_contrato_elem','sending'=>true,'action'=>'call','data'=>0],'tag'=>'Alta de Venta '],
    ...
    public function list(){
      ...
      $vts = [];
      $itms=[];
      $ventas = $this->Mdb->db->query("SELECT * FROM VENTAS ORDER BY id ASC ");
  		if($ventas->result_id->num_rows){
        // var_dump($ventas->result_array()); exit();
        foreach ($ventas->result_array() as $v) {

          $x = $this->Mdb->db->query("SELECT * FROM ITEMS_VENTAS WHERE owner_id = {$v['owner_id']} AND venta_id = {$v['id']}");
          if($x->result_id->num_rows){
            $v['items'] = $x->result_array();
          }
          $vts[]=$v;
        }
      }
      $th = [
        ['label'=>'fecha','title'=>'Fecha'],
        ['label'=>'cliente','title'=>'Cliente'],
        ['label'=>'vendedor','title'=>'Vendedor'],
        ['label'=>'total','title'=>'Total $'],
        ['label'=>'comision_x_unidad','title'=>'Comision x Unid.'],
        ['label'=>'cant_productos','title'=>'Prods. Vendidos'],
        ['label'=>'items','title'=>'Desc']
      ];
      $r = [
  			'method'=>'list',
  			'action'=>'response',
    	...
    	function pcle_updv($type=0,$prnt_id=0,$id=0,$v=0){
    		if($type == 0 && $prnt_id == 0 && $id == 0 && $v == 0){
    			$p = $this->input->post();
    			if(array_key_exists('user_id',$p) && array_key_exists('parent_id',$p) && array_key_exists('pcle_id',$p) && array_key_exists('data',$p)){
    				$type = 'ATOM';
    				$prnt_id = $p['parent_id'];
    				$id = $p['pcle_id'];
    				$v = $p['data'];
    		...
    		if($user_id < 500 && $type && $prnt_id && $id && $v){
    			$e = new $type($prnt_id);
    			$e->pcle_updv($id,$v);
    			// return 'ok';
  ...
  //  DEVUELVE EL HTML CON LA LLAMADA A ROUTER.JS Y EL ELEMENT_ID CORRESPONDIENTE A LA CUOTA
    ...
    function get_srvs($elm_id){
      ...
      foreach ($s as $v) {
        ...
        $srv_name = $this->get_srv_name(intval($srv->get_pcle('atom_id')->value));
    ...
    // retorna el nombre del servicio , recibe el atom_id
    function get_srv_name($a){
      $res = '';
      $t = $this->Mdb->db->query("SELECT name FROM atoms WHERE id = {$a} ");
    ...
    // CREA DOS SERVICIOS DE REFINANCIACION
      ...
      if(is_array($p)){
        ...
        if(array_key_exists('srv_events_id',$p)){
          $data_refi_srvc = [
            'elm_id'=>$p['elm_id'],
            'fields'=>[
              'atom_id'=>12659,
    ...
    function get_uploaded_files($elm_id,$folder){
      ...
      function get_last_movs($elm_id){
        // ** OBTENGO DATOS
        $m = $this->app_model->get_arr("SELECT IF(DATE_FORMAT(c.fecha_comprobante,'%d/%m/%Y')!= '00/00/0000',DATE_FORMAT(c.fecha_comprobante,'%d/%m/%Y'), DATE_FORMAT(c.fecha,'%d/%m/%Y'))  as 'fecha_recibo',IF(a.observaciones != '',a.observaciones,  REPLACE(c.concepto,'pago cuota Nro. ','')) as 'concepto', (c.monto) as 'monto_operacion', c.nro_comprobante as 'recibo_nro' FROM `comprobantes` c LEFT OUTER JOIN contab_asientos a on a.operacion_nro = c.op_caja_nro WHERE c.estado > 0 AND c.elements_id = {$elm_id} AND c.nro_comprobante > 0 ORDER BY c.nro_comprobante DESC ");
        //  SI HAY DATOS
      ...
      function detalle_movs(){
        ...
        if(empty($events)){
          ...
          //  RETORNA LOS PLCES DEL EVENT CON dom_types > 0
      ...
      */
      ...
      // EN REVISION VA A QUEDAR DEPRECADO LO REEMPLAZA ENVIAR MENSAJE
      function set_curr_state(){
        $p = $this->input->post();
        // $t = $this->rescindir_contrato($p);
        // $this->cmn_functs->resp('check',$t);
        // exit;

        // actualizo el pcle del contrato
        $e = new Element($p['elem_id']);
        $e->set_pcle(0,'curr_state',$p['state'],$title ='Estado',$vet = -1);
        ...
        switch ($p['state']) {
          ...
          case 'EN_RESCISION':
            // RESCINDIR CONTRATO CREA UN ATOM RESCISION Y DEVUELVE EL ID PARA SETEAR EL CONTRATO
            $r_id = $this->rescindir_contrato($p);
            $e->set('owner_id',$r_id);
            $e->pcle_updv($e->get_pcle('prod_id')->value,$r_id);

            //   RESPONDER AL FRONT
            $aft_act = ['method'=>'get_elements','sending'=>true,'data'=>['elm_id'=>$e->id]];
            $r =[
              'tit'=>'Rescision de contrato ',
              'msg'=>'El Contrato del lote '.$p['lote_nom']. ' fue rescindido. '  ,
              'type'=>'warning',
              'container'=>'modal',
      ...
      //*** modif 13/04/2020
      function rescindir_contrato($p){

        //  CREATE ATOM RESCISION
        $r = new atom(0,'RESCISION','R_'.$p['lote_nom']);
        $pcles = $r->get_pcle();
        foreach ($pcles as $pcl){
          // $log[]=['array_search'=>array_key_exists($pcl->label, $p['data']),'pcl_id'=>$pcl->id,'p_lbl'=>$pcl->label,'data'=>$p['data']];
          if(array_key_exists($pcl->label, $p['data'])){
            $r->set_pcle($pcl->id,$pcl->label,$p['data'][$pcl->label]);
          }
        }
        ...
        // modifica elements_types_id a  rescicion type
        $e->set('elements_types_id',5);
      ...
      // *** FUNCIONES DE VERSION 2 DE FINANCIACION
        ...
        // COLECTA el primer elemento A PAGAR Y LOS COVIERTE EN  VENCID SI SUPERA
        ...
        function update_vencimientos($e){
      		...
      		$f = $e->get_events_first_future();
      		if(!empty($f['events'])){
      			$dt_xv = new DateTime(substr($f['events']['fecha'],0,8).'01');
      			$dt_diff = $dt_xv->diff($dt_now);
      			if($dt_diff->invert == 0){
      				if($dt_diff->days >= 25){
      					$ev = new Event(intval($f['events']['id']));
    ...
    // *************************************************************************
    // ******* PREPARA LA VENTANA DE NUEVA VENTA
    // *************************************************************************
    function call_new_elem(){
        $p = $this->input->post('data');
        $struct = $this->app_model->get_arr("SELECT label,value,title,dom_types,vis_ord_num FROM `elements_struct` WHERE elements_types_id = {$p['elements_types_id']} AND vis_ord_num > 0 ORDER BY vis_ord_num ASC");
        if($struct){
          switch ($p['elements_types_id']) {
            case '4':
              //  fix label atom_id to servicios
              foreach ($struct as $key => $s) {
               $struct[$key]['dom_types'] = $this->cmn_functs->get_vis_elem_name($s['dom_types']);
            ...
            default:
              ...
              foreach ($struct as $key => $st) {
                $struct[$key]['dom_types'] = $this->cmn_functs->get_vis_elem_name($st['dom_types']);
                if($st['vis_ord_num'] == 99){unset($struct[$key]);}
              }
              $prod_id_precio = 0;
              $t = $this->Mdb->db->query("SELECT a.id,ap.value as precio_unit, apc.value as cant_disponible FROM `atoms` a
                JOIN atoms_pcles ap on ap.atom_id = a.id AND ap.label = 'precio_unitario'
                JOIN atoms_pcles apc on apc.atom_id = a.id AND apc.label = 'cant_disponible'
                WHERE a.atom_types_id = 2");
              if($t->result_id->num_rows){
                $prod_id_precio = $t->result_array();
              }
              break;
          }
          $this->cmn_functs->resp('front_call',[
    ...
    function save_venta(){
      ...
      $p = $this->input->post('data');
      if(empty($p) || empty($p['owner_id']) || empty($p['total'])|| empty($p['sale_prod'])){
        $this->cmn_functs->resp('front_call',[
        ...
        $v = new Element(-1,"VENTAS",$p['owner_id']);
        $v->pcle_updv($v->get_pcle('fecha')->id,$p['fecha']);
        $v->pcle_updv($v->get_pcle('owner_id')->id,$p['owner_id']);
        $v->pcle_updv($v->get_pcle('monto_total')->id,$p['total']);
        $v->pcle_updv($v->get_pcle('vendedor')->id,$p['vendedor']);
        $v->pcle_updv($v->get_pcle('comision')->id,$p['comision']);

        $total_items_vendidos = 0;
        foreach ($p['sale_prod'] as $s) {
          $total_items_vendidos += intval($s['cant']);
          $sp = new Element(-1,'PRODS_VENDIDOS',$v->id);
          $sp->pcle_updv($sp->get_pcle('prod_id')->id,$s['prod_id']);
          $sp->pcle_updv($sp->get_pcle('cantidad')->id,$s['cant']);
          $sp->pcle_updv($sp->get_pcle('precio_venta')->id,$s['precio']);
          $sp->pcle_updv($sp->get_pcle('subtotal')->id,$s['subtot']);
          $sp->pcle_updv($sp->get_pcle('owner_id')->id,$p['owner_id']);

          $prod = new Atom($s['prod_id']);
          $curr_cant = intval($prod->get_pcle('cant_disponible')->value);
          $prod->pcle_updv($prod->get_pcle('cant_disponible')->id,$curr_cant - intval($s['cant']));

        }

        //*** ASIENTO DE CAJA
        $monto_comisiones = intval($p['comision']) * $total_items_vendidos;
        $monto_total = intval($p['total']);

        // **** FILL DE DATOS PARA EL ASIENTO de caja
        $c = [
          'nro_comprobante'=> 0,
          'cuentas_id' => 3,
          'cliente_id'=>$p['owner_id'],
          'venta_id'=>$v->id,
          'operador_usuario_id'=>0,
          'origen'=>'save_venta',
          'tipo_asiento'=>'INGRESOS',
          'cuenta_imputacion_id' => 260,
          'monto'=> $monto_total,
          'observaciones'=>''
        ];

        $op_nro=$this->cmn_functs->mk_asiento_caja($c);

        $c2 = [
          'nro_comprobante'=> 0,
          'cuentas_id' => 3,
          'proveedor_id'=>$p['vendedor'],
          'venta_id'=>$v->id,
          'operador_usuario_id'=>0,
          'origen'=>'save_venta',
          'tipo_asiento'=>'EGRESOS',
          'cuenta_imputacion_id' => 259,
          'monto'=> $monto_comisiones,
          'observaciones'=>'deducido de operacion Nro'.$op_nro
        ];

        $op_comision_nro=$this->cmn_functs->mk_asiento_caja($c2);
    ...
    function actualizar_contrato(){
      ...
      $struct = $this->app_model->get_arr("SELECT st.label,p.value,st.title,vo.nombre as dom_types,st.vis_ord_num  FROM elements_pcles p JOIN elements_struct st on st.id = p.struct_id JOIN dom_types vo on vo.id = st.dom_types  WHERE elements_id = {$p['elm_id']} ORDER BY st.vis_ord_num ");
      ...
      foreach ($struct as $key => $v) {
        ...
        if($v['label'] === 'prod_id'){
          ...
          $struct[$key]['dom_types'] = 'text_readonly';
        }
    ...
    // *************************************************************************
      ...
      function call_update_plan($e_id=0){
          ...
          if(preg_match_all('!\d+!', $x, $m)){
            ...
            $ctr_data=[
              ['label'=>$pmt->get_pcle('fecha_vto')->label,'value'=>$pmt->get_pcle('fecha_vto')->value,'title'=>'Fecha último vencimiento','dom_types'=>'text','readonly'=>true],
              ['label'=>$pmt->get_pcle('monto_pagado')->label,'value'=>$pmt->get_pcle('monto_pagado')->value,'title'=>'Monto de cuota último pago','dom_types'=>'text','readonly'=>true],
              ['label'=>'last_nro_cta','value'=>$pmt->get_pcle('nro_cta')->value,'title'=>'Nro. Cuota último pago','dom_types'=>'text','readonly'=>true],
              ['label'=>'cant_ctas_imputadas','value'=>$e->get_cant_ctas_imputadas(),'title'=>'Total Ctas. Imputadas','dom_types'=>'text','readonly'=>true],
              // ['label'=>'cant_ctas_restantes','value'=>$e->get_pcle('cant_ctas_restantes')->value,'title'=>'Cuotas. Restantes','dom_types'=>'text','readonly'=>true],

              ['label'=>'saldo_a_financiar','value'=>$saldo_un_pago ,'title'=>'Saldo a Financiar','dom_types'=>'text','readonly'=>($this->usr_obj->permisos_usuario >1)?true:false],

              ['label'=>'monto_cta_1','value'=>intval($pmt->get_pcle('monto_pagado')->value) ,'title'=>'Monto Proxima Cuota','dom_types'=>'text'],
            ];
            // 'readonly'=>($this->usr_obj->permisos_usuario >1)?true:false]
            //FIELDS DEL CONTRATO
            if(array_key_exists('arbitrary_call', $p)){
              $ctr_fields = [['label'=>'cant_ctas_restantes','value'=>$ctas_restantes_div,'title'=>'Cuotas. Restantes','dom_types'=>'text','readonly'=>true]
              ];
            }else{
              $ctr_fields = [['label'=>'cant_ctas_restantes','value'=>$ctas_restantes_div,'title'=>'Cuotas. Restantes','dom_types'=>'text','readonly'=>true]
              ];
      ...
      function save_update_plan(){
        ...
        // SI ESTA EN CICLO 1  ACTUALIZA CURRENT CICLO RESTANDO LAS CUOTAS DE CICLO 2 A LAS RESTANTES DEL CICLO ACTUAL
        // if($elem->get_pcle('current_ciclo')->value == 1){
        //   $cant_ctas_restantes = intval($elem->get_pcle('cant_ctas_restantes')->value) - intval($elem->get_pcle('cant_ctas_ciclo_2')->value);
        // }
    ...
    public function autocomplete_lotes_vendidos(){
      ...
      foreach ($lid as $l) {
        ...
        $cli = false;
        $tit = false;
        if(!empty($e->get_pcle('cli_id'))){
          $cli =$e->get_pcle('cli_id')->value;
        }else if(!empty($e->get_pcle('titular_id'))){
  ...
  /* no usados
    ...
    // ****  ATOM CRUDE
      function add(){
        ...
        foreach ($p['data'] as $d) {
          $this->app_model->insert('atoms_pcles',array('atom_id'=>$p['atom_id'],'label'=>$d['label'],'value'=>$d['value'],'title'=>$d['title'],'dom_types'=>$d['dom_types']));
        }
        $r =[
